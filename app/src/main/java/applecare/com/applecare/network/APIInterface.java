...
public interface APIInterface {
    ...
    @GET("questions/")
    Call<ResponseBody> getQuestions(@Header("Authorization") String authorization );
    @GET("questions/")
    Call<ResponseBody> getQuestions(@Header("Authorization") String authorization, @Query("filter") String filter);
    ...
    @GET("{mobile}/AUTOGEN/Reset")
    Call<OTP> getOTP(@Path("mobile") String mobile);

    @GET("VERIFY/{sessionId}/{otp}")
    Call<OTP> verifyOTP(@Path("sessionId") String sessionId, @Path("otp") String otp);

    @FormUrlEncoded
    @PATCH("reset-password/")
    Call<User> resetPassword(@Field("username") String mobileNumber, @Field("password") String password, @Header("TOKEN") String authorization);
}
